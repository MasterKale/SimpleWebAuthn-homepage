...
`"webauthn"` **can** be combined with other typical `autocomplete` values, including `"username"` and `"password"`, but must appear at the end to consistently trigger conditional UI across browsers. The following are all valid:
