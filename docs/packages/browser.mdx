...
As of SimpleWebAuthn v11.0.0 both the `startRegistration()` and `startAuthentication()` accept a single object argument, with various options specified as properties in that argument. This error is likely caused by passing in options directly (the old way), without wrapping them in an object and specifying them as `optionsJSON` as expected.
