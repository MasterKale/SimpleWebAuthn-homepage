...
The following methods represent the "core" methods exported from **@simplewebauthn/browser**:
...
The following WebAuthn signals can be sent using `sendSignal()`:

#### Unknown Credential {/* #sendsignal-unknown-credential */}

This signal communicates that the credential that the user just tried to register, or to authenticate with, was not one that the Relying Party recognizes.

It is a good idea for a Relying Party to send this signal **immediately after the use of an unrecognized credential**. For example, after rejecting the output from [`startRegistration()`](#startregistration) due to unsatisfied RP-specific authenticator registration policy; or after rejecting the output from [`startAuthentication()`](#startauthentication) because the user deleted the corresponding passkey from their Relying Party user account settings page:

```ts
import { sendSignal, type SendSignalUnknownCredentialOpts } from '@simplewebauthn/browser';
...
The credential manager responsible for the credential can hide or delete the credential so that the user does not see it in the future as an option to sign in with.

#### All Accepted Credentials {/* #sendsignal-all-accepted-credentials */}

This signal communicates the current list of passkeys the Relying Party will recognize for use by the **authenticated** user on the next login.

It is a good idea for a Relying Party to **periodically** send this signal, for example **after every successful authentication**:

```ts
import { sendSignal, type SendSignalAllAcceptedCredentialsOpts } from '@simplewebauthn/browser';

const opts: SendSignalAllAcceptedCredentialsOpts = {
  signalName: 'allAcceptedCredentials',
  rpID: 'example.com',
  userID: 'Lrk2ki-arSSFcm6PPlMKEQ',
  allAcceptedCredentialIDs: ['d77Hxqvltn66l7hegq_E3Q', 'nz_ajzNrMq3KfPrkOt3lHQ'],
};
await sendSignal(opts);
```

:::warning[A word of caution about `userID`]
`userID` above **must** be set to the same value for the currently authenticated user as `options.user.id` was in [the options returned from **@simplewebauthn/server**'s `generateRegistrationOptions()`](server.md#1-generate-registration-options) when the passkeys in `allAcceptedCredentialIDs` were generated.
:::

Credential managers that have a passkey for (`rpID` + `userID`), but the passkey ID is not found in `allAcceptedCredentialIds`, may choose to hide or delete the passkey because it will not be accepted for use by the Relying Party.

#### Current User Details {/* #sendsignal-current-user-details */}

This signal communicates a change in the **authenticated** user's name and/or display name.

It is a good idea for a Relying Party to periodically send this signal, for example **after every successful authentication** and **immediately after the user's user name and/or display name is changed**:

```ts
import { sendSignal, type SendSignalCurrentUserDetailsOpts } from '@simplewebauthn/browser';
...
This can help browsers and platforms display the most up-to-date information about the user during a passkey authentication instead of always showing whatever value was set at the time of registration.

#### Generating options with @simplewebauthn/server {/* #sendsignal-generating-options-with-simplewebauthnserver */}

The WebAuthn Signal APIs were all intentionally designed to accept **options that are JSON-serializable**, avoiding `ArrayBuffer` values for Base64URL-encoded `string` values instead. This means that **@simplewebauthn/server** does _not_ have a corresponding `sendSignal()` options generation method.

Instead, **the same types** that @simplewebauthn/browser uses for the various `sendSignal()` options can be **imported directly from @simplewebauthn/server** too:
...
These types can then be used to define project-specific options generation logic on the server that yields values that can be easily passed to the browser as JSON and then passed into `sendSignal()` without modification.

## Feature Detection

The following methods assist in detecting browser and/or OS support for passkeys:
...
The `WebAuthnError` class is used to wrap errors that are raised when calling WebAuthn API methods via [`startRegistration()`](#startregistration), [`startAuthentication()`](#startauthentication), or [`sendSignal()`](#sendsignal). Projects can import this error to perform `instanceof` checks when catching errors to discern SimpleWebAuthn library errors from other errors that may arise.
